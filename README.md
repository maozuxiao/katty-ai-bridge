...
1. **供应商**：从内置预设里选一家，填 API Key，选模型。自定义端点（公司内网网关、Ollama、codebuddy 等）直接填 API 地址即可——`127.0.0.1` / `localhost` / `[::1]` / `10.*` / `192.168.*` / `172.16-31.*` / `*.local` 会被**自动判定为免 API Key**；若该端点其实需要鉴权，勾回「此端点需要 API Key」即可
